class MakeupsController < ApplicationController
  ...
  # GET /makeups or /makeups.json
  def index
    @makeups = Makeup.all
  end

  # GET /makeups/1 or /makeups/1.json
  def show
  end
  ...
  # POST /makeups or /makeups.json
  def create
    @makeup = Makeup.new(makeup_params)

    respond_to do |format|
      if @makeup.save
        format.html { redirect_to makeup_url(@makeup), notice: "Makeup was successfully created." }
  ...
  # PATCH/PUT /makeups/1 or /makeups/1.json
  def update
    respond_to do |format|
      if @makeup.update(makeup_params)
        format.html { redirect_to makeup_url(@makeup), notice: "Makeup was successfully updated." }
  ...
  # DELETE /makeups/1 or /makeups/1.json
  def destroy
    @makeup.destroy
  ...
  private
    ...
    def makeup_params
      params.require(:makeup).permit(:name, :rating, :colour)
    end
end
